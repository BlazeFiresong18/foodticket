...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadUsers() {
    ...
    post("/api/admin/users", {}).then(function (data) {
      if (!guard(data)) return;
      if (data.status !== "success") { setStatus("Failed to load users.", true); return; }
      setStatus("");
      var rows = data.users.map(function (u) {
        return (
          ...
          "<td>" + u.id + "</td>" +
          '<td><input class="u-email" value="' + esc(u.email) + '" size="24"/></td>' +
          '<td><input class="u-name" value="' + esc(u.name) + '" size="16"/></td>' +
          '<td><select class="u-role">' +
          ["customer", "scanner", "admin"].map(function (r) {
            return '<option value="' + r + '"' + (u.role === r ? " selected" : "") + ">" + r + "</option>";
          }).join("") +
          "</select></td>" +
          '<td><input type="checkbox" class="u-active"' + (u.is_active ? " checked" : "") + "/></td>" +
          '<td><input class="u-pass" type="password" placeholder="(unchanged)" size="10"/></td>' +
          "<td>" + esc(u.qr_issued_at || "never") + "</td>" +
      ...
      content.innerHTML =
        ...
        "<th>New password</th><th>QR issued</th><th></th></tr>" + rows + "</table>" +
      ...
      content.querySelectorAll(".u-save").forEach(function (btn) {
        btn.onclick = function () {
          ...
          post("/api/admin/user-update", {
            ...
            is_active: tr.querySelector(".u-active").checked ? "true" : "false",
            password: tr.querySelector(".u-pass").value
          }).then(function (d) {
            if (!guard(d)) return;
            if (d.status === "success") { setStatus("Saved."); loadUsers(); }
      ...
      content.querySelectorAll(".u-del").forEach(function (btn) {
        btn.onclick = function () {
          ...
          post("/api/admin/user-delete", { id: tr.dataset.id }).then(function (d) {
            if (!guard(d)) return;
            if (d.status === "success") { setStatus("Deleted " + email + "."); loadUsers(); }
      ...
      function sendQr(ids, label) {
        setStatus("Sending QR codes " + label + " — this can take a while...");
      ...
      document.getElementById("qr-all").onclick = function () {
        if (confirm("Send a fresh QR code to ALL active customers? Existing codes are invalidated."))
      ...
      document.getElementById("qr-selected").onclick = function () {
        ...
        if (!ids.length) { setStatus("Select at least one user first.", true); return; }
        if (confirm("Send fresh QR codes to " + ids.length + " user(s)? Their existing codes are invalidated."))
      ...
      document.getElementById("new-create").onclick = function () {
        ...
        post("/api/admin/user-create", {
          ...
          if (!guard(d)) return;
          if (d.status === "success") { setStatus("User created."); loadUsers(); }
  ...
  // ── Scan log tab ────────────────────────────────────────────────────
  var FLAGGED = { invalid_token: 1, cooldown: 1, inactive_user: 1, otp_failed: 1 };

  function loadScans() {
    setStatus("Loading scan log...");
    post("/api/admin/scan-logs", {}).then(function (data) {
      if (!guard(data)) return;
      if (data.status !== "success") { setStatus("Failed to load scan log.", true); return; }
      setStatus("");
      var rows = data.logs.map(function (l) {
        var cls = FLAGGED[l.result] ? ' class="row-flagged"' : "";
        return "<tr" + cls + "><td>" + esc(l.at) + "</td><td>" + esc(l.scanner) +
          "</td><td>" + esc(l.customer_name || "—") + "<br/><small>" + esc(l.customer_email) +
          "</small></td><td>" + esc(l.result) + "</td><td><small>" + esc(l.payload) + "</small></td></tr>";
      }).join("");
      content.innerHTML =
        ...
        "<th>Result</th><th>Payload</th></tr>" + rows + "</table>";
    }).catch(function () { setStatus("Network error.", true); });
  }
  ...
  function loadAudit() {
    ...
    post("/api/admin/audit-logs", {}).then(function (data) {
      if (!guard(data)) return;
      if (data.status !== "success") { setStatus("Failed to load audit log.", true); return; }
      setStatus("");
      var rows = data.logs.map(function (l) {
        return "<tr><td>" + esc(l.at) + "</td><td>" + esc(l.admin) + "</td><td>" + esc(l.action) +
          "</td><td>" + esc(l.target) + "</td><td><small>" + esc(l.details) + "</small></td></tr>";
      }).join("");
      content.innerHTML =
        '<table class="data-table"><tr><th>Time</th><th>Admin</th><th>Action</th>' +
        "<th>Target</th><th>Details</th></tr>" + rows + "</table>";
    }).catch(function () { setStatus("Network error.", true); });
  }
