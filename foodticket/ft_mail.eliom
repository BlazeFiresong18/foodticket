...
let smtp_config () =
  Letters.Config.create ~username:Ft_config.smtp_user
    ~password:Ft_config.smtp_pass ~hostname:Ft_config.smtp_host
    ~with_starttls:false ()
  |> Letters.Config.set_port (Some Ft_config.smtp_port)

let send_plain ~to_email ~subject ~text =
  match
    Letters.create_email ~from:Ft_config.smtp_from
      ~recipients:[ Letters.To to_email ] ~subject ~body:(Letters.Plain text)
      ()
  with
  | Ok message ->
    Lwt.catch
      (fun () ->
         ...
         Lwt.return true)
      (fun exn ->
         Ft_log.error "email_send_failed"
           [ ("to", to_email); ("exn", Printexc.to_string exn) ];
         Lwt.return false)
  | Error e ->
...
let qr_mailer_path =
  Ft_config.get_or "FT_QR_MAILER" "services/qr_mailer/qr_mailer.py"
  ...
  Lwt.catch
    (fun () ->
       ...
       | `Assoc fields ->
         ...
         let sent, failed =
           List.fold_left
             (fun (sent, failed) r ->
                ...
                | `Assoc f ->
                  let email =
                    match List.assoc_opt "email" f with
                    | Some (`String e) -> e
                    | _ -> "?"
                  in
                  let ok =
                    match List.assoc_opt "ok" f with
                    | Some (`Bool b) -> b
                    | _ -> false
                  in
         ...
         in
         List.iter
           (fun e -> Ft_log.error "qr_email_failed" [ ("to", e) ])
           failed;
         Lwt.return (sent, List.rev failed)
       | _ ->
         Ft_log.error "qr_mailer_bad_output"
           [ ("output", Ft_util.truncate 500 output) ];
