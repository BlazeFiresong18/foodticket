...
let get_or name default =
  match get name with Some v -> v | None -> default
...
let require name =
  ...
  | None ->
    failwith
      (Printf.sprintf
         "FoodTicket: missing required configuration %s (set the environment \
          variable or add it to .env)"
         name)
